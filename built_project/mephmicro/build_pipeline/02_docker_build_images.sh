...
function calculate_framework_version
{
  ...
  if [[ "$FRAMEWORK_VERSION" == "" ]] ; then
    ...
    NUM_DEPENDENCIES=$(mvn dependency:tree | grep ":" | grep -v mephmicro  | grep -v Total | grep -v Finish | wc -l)
    DEP_SIZE=$(mvn dependency:tree | grep ":" | grep -v mephmicro  | grep -v Total | grep -v Finish | wc -c)
