...
{
  FRAMEWORK_VERSION=$(mvn dependency:tree | grep ":" | grep -v mephmicroapi  | grep -v Total | grep -v Finish | md5)
  if [[ "$FRAMEWORK_VERSION" == "" ]] ; then
    echo "Building image - cannot use hash, looking for simulation of hash ..."
    NUM_DEPENDENCIES=$(mvn dependency:tree | grep ":" | grep -v mephmicroapi  | grep -v Total | grep -v Finish | wc -l)
    DEP_SIZE=$(mvn dependency:tree | grep ":" | grep -v mephmicroapi  | grep -v Total | grep -v Finish | wc -c)
...
function splitJarIntoFrameworkAndCode
{
    ...
    pushd $BASEDOCKERDIR/tmp_for_jars/decomp
      ...
      find . -type f | grep -v lib | grep -v "org/springframework/boot/loader" > ../micro_content.txt;find . -type f | grep lib | grep mephmicroapi >> ../micro_content.txt
      find . -type f | grep "org/springframework/boot/loader" > ../framework_content.txt; find . -type f | grep lib | grep -v mephmicroapi >> ../framework_content.txt
...
if [ -d "../mephmicroapi-restapiApp" ]; then
    echo "Building image - building image for restApi  ($DOCKER_RESTAPI_IMAGE_NAME)"
    build_image_for_java mephmicroapi-restapiApp $DOCKER_RESTAPI_IMAGE_NAME $DOCKER_RESTAPI_FWK_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
...
if [ -d "../mephmicroapi-engineApp" ]; then
    echo "Building image - building image for engine, $DOCKER_RESTAPI_IMAGE_NAME"
    build_image_for_java mephmicroapi-engineApp $DOCKER_ENGINE_IMAGE_NAME $DOCKER_ENGINE_FWK_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Building image - Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
