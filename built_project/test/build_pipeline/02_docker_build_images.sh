...
if [ -d "../test-restapiApp" ]; then
    build_image_for_java test-restapiApp $DOCKER_RESTAPI_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
...
if [ -d "../test-engineApp" ]; then
    build_image_for_java test-engineApp $DOCKER_ENGINE_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
...
if [ -d "../test-singleApp" ]; then
    build_image_for_tomcat test-singleApp $DOCKER_SINGLEAPP_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
