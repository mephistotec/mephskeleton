...
if [ -d "../mephskeleton-restapiApp" ]; then
    build_image_for_java mephskeleton-restapiApp $DOCKER_RESTAPI_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
...
if [ -d "../mephskeleton-engineApp" ]; then
    build_image_for_java mephskeleton-engineApp $DOCKER_ENGINE_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
...
if [ -d "../mephskeleton-singleApp" ]; then
    build_image_for_tomcat mephskeleton-singleApp $DOCKER_SINGLEAPP_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
