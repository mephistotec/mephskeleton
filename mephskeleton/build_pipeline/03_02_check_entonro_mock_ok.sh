...
while [  $COUNTER -lt 50 ]; do
    ...
    if [ -d "../mephskeleton-restapiApp" ]; then
        STATUS_RESTAPI=$(docker run --network=$netname --rm appropriate/curl -fsSL http://mephskeleton_restapi_external:8080/health | jq  -r ".status");
        echo "------- VALIDAMOS RESTAPI $STATUS_RESTAPI -------"
    fi;
    if [ -d "../mephskeleton-engineApp" ]; then
        STATUS_ENGINE=$(docker run --network=$netname --rm appropriate/curl -fsSL http://mephskeleton_engine_external:8080/health | jq -r ".status");
        echo "------- VALIDAMOS ENGINE $STATUS_ENGINE -------"
    fi;
    if [ -d "../mephskeleton-singleApp" ]; then
        STATUS_TOMCAT=$(docker run --network=$netname --rm appropriate/curl -fsSL http://mephskeleton_singleapp_external:8080/health | jq -r ".status");
