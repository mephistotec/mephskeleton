...
versionTag=$(echo "$CURRENT_BRANCH"_"$STACK_VERSION" |  sed "s/\./\\\\./g" | sed "s/\//_/g" )

echo "DeployStack - Cleaning previous descriptor for $environment"
mkdir -p ../k8s/$environment
rm ../k8s/$environment/*
rc=0;
echo "DeployStack - Building k8s descriptors for environment $environment with tag $versionTag"
pushd ./stack_definitions/k8s_templates
    ...
    do
        if [[ $rc -eq 0 ]] ; then
            ...
            sed "s/<deploymentVersionTag>/$versionTag/g" |
