...
FILE_ZIP=$BASEZIPDIR/mephskeleton-$ARTIFACT_VERSION.zip

export AWS_ACCESS_KEY_ID=$CICD_S3_USER
export AWS_SECRET_ACCESS_KEY=$CICD_S3_PASS

echo "Copiamos $BASEZIPDIR/mephskeleton-$ARTIFACT_VERSION.zip a S3 ($S3_LSDOMAINS_PATHpw) con $AWS_ACCESS_KEY_ID"
echo "--------------------------"

# volcamos los parametros para que los recoja el jeninsfile
echo "mephskeleton" > ./tmp/zip_s3_files/LAST_GROUPNAME.txt
echo "mephskeleton-$ARTIFACT_VERSION.zip" > ./tmp/zip_s3_files/LAST_ARTIFACT.txt

aws s3 cp $FILE_ZIP  s3://$S3_LSDOMAINS_PATH/mephskeleton-$ARTIFACT_VERSION.zip
