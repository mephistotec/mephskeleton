...
function pullPushZip
{
   ...
   FILE_ZIP=$BASEZIPDIR/mephskeleton-$pi_VERSION.zip
    ...
    echo "Copiamos $BASEZIPDIR/mephskeleton-$pi_VERSION.zip de  S3 ($pi_S3_ORIGEN_URL) a ($pi_S3_DESTINO_URL) con $AWS_ACCESS_KEY_ID"

    aws s3 cp $pi_S3_ORIGEN_URL/mephskeleton-$pi_VERSION.zip ./tmp/zip_s3_files/
    rc=$?
    if [[ $rc -ne 0 ]] ; then
      echo "--- Error Recuperando version origen $BASEZIPDIR/mephskeleton-$pi_VERSION.zip de  S3 ($pi_S3_ORIGEN_URL) !!!"; exit $rc
    fi
    ...
    aws s3 cp ./tmp/zip_s3_files/mephskeleton-$pi_VERSION.zip $pi_S3_DESTINO_URL/mephskeleton-$pi_VERSION.zip
    rc=$?
    if [[ $rc -ne 0 ]] ; then
