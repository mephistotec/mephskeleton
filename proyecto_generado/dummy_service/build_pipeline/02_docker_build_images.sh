...
if [ -d "../dummy_service-restapiApp" ]; then
    build_image_for_java dummy_service-restapiApp $DOCKER_RESTAPI_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
...
if [ -d "../dummy_service-engineApp" ]; then
    build_image_for_java dummy_service-engineApp $DOCKER_ENGINE_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
...
if [ -d "../dummy_service-singleApp" ]; then
    build_image_for_tomcat dummy_service-singleApp $DOCKER_SINGLEAPP_IMAGE_NAME
    rc=$?
    if [[ $rc -ne 0 ]] ; then
       echo "Bulild IMAGE ERROR error : $rc"; exit $rc
    fi
