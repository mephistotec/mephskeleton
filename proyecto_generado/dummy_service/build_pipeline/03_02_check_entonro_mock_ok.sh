...
while [  $COUNTER -lt 50 ]; do
    ...
    if [ -d "../dummy_service-restapiApp" ]; then
        STATUS_RESTAPI=$(docker run --network=$netname --rm appropriate/curl -fsSL http://dummy_service_restapi_external:8080/health | jq  -r ".status");
        echo "------- VALIDAMOS RESTAPI $STATUS_RESTAPI -------"
    fi;
    if [ -d "../dummy_service-engineApp" ]; then
        STATUS_ENGINE=$(docker run --network=$netname --rm appropriate/curl -fsSL http://dummy_service_engine_external:8080/health | jq -r ".status");
        echo "------- VALIDAMOS ENGINE $STATUS_ENGINE -------"
    fi;
    if [ -d "../dummy_service-singleApp" ]; then
        STATUS_TOMCAT=$(docker run --network=$netname --rm appropriate/curl -fsSL http://dummy_service_singleapp_external:8080/health | jq -r ".status");
