...
NAME_ZIP=mephmicro-batch-$ARTIFACT_VERSION-zip.zip
FILE_ZIP=$BASEZIPDIR/$NAME_ZIP

export AWS_ACCESS_KEY_ID=$CICD_S3_USER
export AWS_SECRET_ACCESS_KEY=$CICD_S3_PASS

echo "Copiamos $BASEZIPDIR/mephmicro-$ARTIFACT_VERSION.zip a S3 ($S3_LSDOMAINS_PATH) con $AWS_ACCESS_KEY_ID"
echo "--------------------------"

# volcamos los parametros para que los recoja el jeninsfile
# volcamos los parametros para que los recoja el jeninsfile
echo "softbase" > ./tmp/zip_s3_files/LAST_GROUPNAME_WDPRE.txt
echo "mephmicro" > ./tmp/zip_s3_files/LAST_ARTIFACT_WDPRE.txt
echo "$ARTIFACT_VERSION" > ./tmp/zip_s3_files/LAST_VERSION_WDPRE.txt

aws s3 cp $FILE_ZIP  s3://$S3_LSDOMAINS_PATH/mephmicro-batch-$ARTIFACT_VERSION.zip
