...
while [  $COUNTER -lt 50 ]; do
    ...
    if [ -d "../mephmicro-restapiApp" ]; then
        STATUS_RESTAPI=$(docker run --network=$netname --rm appropriate/curl -fsSL http://mephmicro_restapi_external:8080/health | jq  -r ".status");
        echo "------- VALIDAMOS RESTAPI $STATUS_RESTAPI -------"
    fi;
    if [ -d "../mephmicro-engineApp" ]; then
        STATUS_ENGINE=$(docker run --network=$netname --rm appropriate/curl -fsSL http://mephmicro_engine_external:8080/health | jq -r ".status");
        echo "------- VALIDAMOS ENGINE $STATUS_ENGINE -------"
    fi;
    if [ -d "../mephmicro-singleApp" ]; then
        STATUS_TOMCAT=$(docker run --network=$netname --rm appropriate/curl -fsSL http://mephmicro_singleapp_external:8080/health | jq -r ".status");
